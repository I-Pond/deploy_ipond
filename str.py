...
def predict_water(ph, temperature, turbinity):
    # create a dataframe with the user input
    input_data = pd.DataFrame([[ph, temperature, turbinity]], 
...
st.header('Bagaimana Kualitas air hari ini?')
ph = st.number_input('pH:')
temperature = st.number_input('Temperature:')
turbinity = st.number_input('Turbinity:')


if st.button('Predict'):
    prediction = predict_water(ph, temperature, turbinity)
    if prediction == 0 :
        st.write('Bersih')
    elif prediction == 1:
        st.write('Keruh')
    else: 
        st.write('Tidak Terdefinisi')
    data = {
        'pH': int(ph),
        'Temperature' : int(temperature),
        'Turbinity': int(turbinity),
